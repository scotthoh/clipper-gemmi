...
namespace clipper
{
  ...
  */
  template <class T>
  class HKL_data : public HKL_data_base
  {
  public:
    //! null constructor
    HKL_data() {}
    ...
    {
      T datum;
      const int index = hkl_info().index_of(hkl);
      if (index != -1)
        datum = list[index];
      else
        get_data(hkl, datum);
      datum.data_export(array);
    }
    void data_import(const HKL &hkl, const xtype array[])
    {
      T datum;
      datum.data_import(array);
      set_data(hkl, datum);
    }
    ...
    {
      for (HKL_info::HKL_reference_index ih = parent_hkl_info->first(); !ih.last(); ih.next())
        list[ih.index()] = op(ih);
    }
    ...
    {
      for (HKL_info::HKL_reference_index ih = parent_hkl_info->first(); !ih.last(); ih.next())
        list[ih.index()] = op(ih, src1[ih], src2[ih]);
    }
  ...
  template <class T>
  HKL_data<T>::HKL_data(const HKL_info &hkl_info)
  {
  ...
  template <class T>
  HKL_data<T>::HKL_data(const HKL_info &hkl_info, const Cell &cell)
  {
    init(hkl_info, cell);
  }
  ...
  template <class T>
  HKL_data<T>::HKL_data(const Spacegroup &spacegroup, const Cell &cell, const HKL_sampling &hkl_sampling)
  {
  ...
  template <class T>
  HKL_data<T>::HKL_data(const HKL_data_base &hkl_data)
  {
    init(hkl_data);
  }
  ...
  /*! Initialise the object using a given reflection list and cell.
    ...
    \param cell The unit cell for this datalist. */
  template <class T>
  void HKL_data<T>::init(const HKL_info &hkl_info, const Cell &cell)
  {
  ...
  /*! Initialise the object using a given spacegroup, cell, and sampling.
    ...
    \param hkl_sampling The reflection list description. */
  template <class T>
  void HKL_data<T>::init(const Spacegroup &spacegroup, const Cell &cell, const HKL_sampling &hkl_sampling)
  {
  ...
  \param hkl_data The HKL_data object to provide the data. */
  template <class T>
  void HKL_data<T>::init(const HKL_data_base &hkl_data)
  {
  ...
  template <class T>
  void HKL_data<T>::mask(const HKL_data_base &mask)
  {
    T null;
    null.set_null();
  ...
  template <class T>
  T HKL_data<T>::operator[](const HKL_info::HKL_reference_coord &ih) const
  {
    if (ih.index() < 0)
    {
      T null;
  ...
  template <class T>
  bool HKL_data<T>::get_data(const HKL_info::HKL_reference_coord &ih, T &data) const
  {
    if (ih.index() < 0)
    {
      data.set_null();
  ...
  template <class T>
  bool HKL_data<T>::set_data(const HKL_info::HKL_reference_coord &ih, const T &data)
  {
    if (ih.index() < 0)
      return false;
  ...
  /*! If a symmetry mate of the requested HKL exists in the list, then
    ...
    \return The data, or 'missing'. */
  template <class T>
  T HKL_data<T>::operator[](const HKL &hkl) const
  {
    int index, sym;
    bool friedel;

    index = parent_hkl_info->index_of(parent_hkl_info->find_sym(hkl, sym, friedel));
    if (index < 0)
    {
      T null;
      null.set_null();
  ...
  /*! If a symmetry mate of the requested HKL exists in the list, then
    ...
    \return true if the data was returned. */
  template <class T>
  bool HKL_data<T>::get_data(const HKL &hkl, T &data) const
  {
    int index, sym;
    bool friedel;
  ...
  /*! If a symmetry mate of the requested HKL exists in the list, then
    ...
    \return true if the data was set. */
  template <class T>
  bool HKL_data<T>::set_data(const HKL &hkl, const T &data_)
  {
    int index, sym;
    bool friedel;
    index = parent_hkl_info->index_of(parent_hkl_info->find_sym(hkl, sym, friedel));
  ...
  /*! The data list is copied from the assignment source to the
    ...
    \return This list. */
  template <class T>
  HKL_data<T> &HKL_data<T>::operator=(const HKL_data<T> &other)
  {
    if (parent_hkl_info == NULL)
    {
      init(other);
  ...
  template <class T>
  HKL_data<T> &HKL_data<T>::operator=(const T &value)
  {
    for (unsigned int i = 0; i < list.size(); i++)
      list[i] = value;
    return *this;
