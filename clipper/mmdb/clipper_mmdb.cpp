...
namespace clipper
{
  ...
  {
    if (!Ter && WhatIsSet & ::mmdb::ASET_Occupancy)
      return mmdb::CAtom::occupancy;
    else
      return Util::nan();
  }
  ...
  {
    if (!Ter && WhatIsSet & ::mmdb::ASET_tempFactor)
      return Util::b2u(tempFactor);
    else
      return Util::nan();
  }
  ...
  {
    if (!Ter && WhatIsSet & ::mmdb::ASET_OccSigma)
      return sigOcc;
    else
      return Util::nan();
  }
  ...
  {
    if (!Ter && WhatIsSet & ::mmdb::ASET_tFacSigma)
      return Util::b2u(sigTemp);
    else
      return Util::nan();
  }
  ...
  {
    ::mmdb::InitMatType();
    Message::message(Message_ctor(" [MMDBManager: constructed>"));
  }
  ...
  Spacegroup MMDBManager::spacegroup() const
  {
    ...
    { // get spacegroup from ops
      int nops = mmdb.GetNumberOfSymOps();
      String ops = "";
      for (int i = 0; i < nops; i++)
        ops += String(mmdb.GetSymOp(i)) + ";";
      Spgr_descr sd(ops, Spacegroup::Symops);
      sg = Spacegroup(sd);
    }
    else
    { // otherwise get spacegroup from name
