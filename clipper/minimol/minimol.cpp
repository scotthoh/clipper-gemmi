...
namespace clipper
{
  ...
  void MAtom::set_id(const String &s, const bool is_gemmi) { id_ = id_tidy(s, is_gemmi); }

  void MAtom::set_name(const String s, const String altconf, const bool is_gemmi)
  {
    if (altconf != "")
      set_id(s + ":" + altconf, is_gemmi);
    else
      set_id(s, is_gemmi);
  }

  String MAtom::id_tidy(const String &id, const bool is_gemmi)
  {
    int pos = id.find(":");
    if (pos == String::npos)
      pos = id.length();
  ...
  bool MAtom::id_match(const String &id1, const String &id2, const MM::MODE &mode)
  {
    if (mode == MM::UNIQUE)
      ...
      return (id1.substr(0, 4) == id2.substr(0, 4));
  }
  ...
  const MAtom &MMonomer::find(const String &n, const MM::MODE mode) const
  {
    int i = lookup(n, mode);
    if (i < 0)
      Message::message(Message_fatal("MMonomer: no such atom"));
    return children[i];
  }
  ...
  void MMonomer::insert(const MAtom &add, int pos)
  {
    if (pos < 0)
      ...
      children.insert(children.begin() + pos, add);
  }
  ...
  const MMonomer &MPolymer::find(const String &n, const MM::MODE mode) const
  {
    int i = lookup(n, mode);
    if (i < 0)
      Message::message(Message_fatal("MPolymer: no such monomer"));
    return children[i];
  }
  ...
  const MPolymer &MModel::find(const String &n, const MM::MODE mode) const
  {
    int i = lookup(n, mode);
    if (i < 0)
      Message::message(Message_fatal("MModel: no such polymer"));
    return children[i];
  }
  ...
  */
  void MMonomer::protein_mainchain_build_carbonyl_oxygen(const MMonomer &next)
  {
    ...
    if (o1 >= 0)
      children.erase(children.begin() + o1);
    // add the atom
  ...
  */
  ...
  int MMonomer::rotamer_find(String res, int rota, TYPE t)
  {
    ...
    if (t == Dunbrack)
      rotamer_data_size = data::rotamer_data_dunbrack_size;
    else
      rotamer_data_size = data::rotamer_data_richardson_size;
    const data::Rotamer_data *rotamer_data;
    if (t == Dunbrack)
      rotamer_data = data::rotamer_data_dunbrack;
    else
      rotamer_data = data::rotamer_data_richardson;
    if (res.length() < 3)
      return 0;
    int p1 = -1;
